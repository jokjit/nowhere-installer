...
1. 在服务端选择「安装 → Portal」。默认版本 `latest`，默认监听 `0.0.0.0:2000`；输入客户端能访问的公网 IP 或域名，密钥留空可自动生成。
2. 选择自动生成证书即可。安装结束时可以显示连接信息，包含完整 Vector URL 和证书 SHA-256 指纹。
3. 在客户端运行同一脚本，选择「安装 → Vector」，填写 Portal 地址、端口、原始共享密钥和证书指纹。默认 SOCKS5 地址为 `127.0.0.1:1080`。
4. 应用程序使用客户端的 SOCKS5 代理。例如在客户端执行：
